...
- 🤖 *Auto-detect AI CLI tools* (Claude Code, OpenCode, Aider, Cursor, and more)
- 📝 *Smart code block formatting* for AI interactions
- 🎯 Buffer-local target tracking with mode-line indicator
...
1. *Set a target pane* (with AI CLI auto-detection):
   #+begin_src
   M-x zellij-set-target-pane
   #+end_src
   - If AI CLIs are detected, you'll see: =Claude Code (Tab #8, Pane 0)=
   - Select your target or choose "Manual selection..."
...
(use-package zellij
  ...
  (zellij-use-project-root t)          ; Use project root as CWD
...
;;    → Suggests: "Claude Code (Tab #8, Pane 0)"
...
;; M-x zellij-send-to-ai
;; → Auto-finds Claude Code/OpenCode/Aider
;; → Formats and sends code
;; → "Sent to Claude Code"
#+end_src
...
- =(zellij-set-target-pane)= - Set buffer-local target with AI detection
